...
Future<void> _fetchFolders() async {
      ...
      if (data['folders'] != null && data['folders'] is List) {
        setState(() {
          folders = List<Map<String, dynamic>>.from(data['folders'].map((item) => {
                ...
                'back_cards_header_title': item['back_cards_header_title'] ?? '',
        ...
        for (var folder in folders) {
          ...
          print('back_cards_header_title: ${folder['back_cards_header_title']}');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Project Folders', style: TextStyle(fontFamily: 'Montserrat', fontWeight: FontWeight.w600)),
        elevation: 0,
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(fontSize: 18, color: Colors.grey[600], fontFamily: 'Roboto'),
                  ),
                ],
              ),
            )
...
class FolderDetailPage extends StatelessWidget {
  ...
  void _showFolderDetails(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          backgroundColor: Color(0xFFF5E6D3).withOpacity(0.9), // Pale sand color with opacity
          title: Text(
            folder['project_title'] ?? 'Unnamed Folder',
          ...
          content: SingleChildScrollView(
            child: Table(
              ...
              children: [
                _buildTableRow('Project Code', folder['project_subject_code'] ?? 'No code'),
                _buildTableRow('Project Description', folder['project_subject_description'] ?? 'No description'),
                _buildTableRow('Start Date', folder['project_start_date'] ?? 'No start date'),
                _buildTableRow('End Date', folder['project_end_date'] ?? 'No end date'),
                _buildTableRow('Module', folder['module_master_name'] ?? 'Unknown'),
                _buildTableRow('Activity', folder['activities_details_content'] ?? 'No activity'),
                _buildTableRow('Card', folder['cards_title'] ?? 'No card'),
                _buildTableRow('Output', folder['outputs_content'] ?? 'No output'),
                _buildTableRow('Instruction', folder['instruction_content'] ?? 'No instruction'),
                _buildTableRow('Coach Detail', folder['coach_detail_content'] ?? 'No coach detail'),
              ],
            ),
          ),
          actions: [
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: Color(0xFFE6D0B3), // Slightly darker shade of pale sand
            ...
            child: SingleChildScrollView(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    if (folder['back_cards_header_title'] != null && folder['back_cards_header_title'].isNotEmpty)
                      Text(
                        'Header: ${folder['back_cards_header_title']}',
                        style: TextStyle(
                          color: Colors.white,
                    ...
                    Text(
                      'Title: ${folder['back_content_title'] ?? 'No title'}',
                      style: TextStyle(
                        color: Colors.white,
                    ...
                    SizedBox(height: 10),  // Slightly increased spacing
                    Text(
                      'Content: ${folder['back_content'] ?? 'No content'}',
                      style: TextStyle(
                        color: Colors.white,
                        fontSize: 20,  // Slightly increased font size
                        height: 1.5,
