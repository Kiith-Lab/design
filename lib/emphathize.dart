...
class Empathize extends StatefulWidget {
  const Empathize({super.key});

  @override
  _EmpathizeState createState() => _EmpathizeState();
}

class _EmpathizeState extends State<Empathize> {
  ...
  Future<void> fetchData() async {
    try {
      List fetchedFrontCards = await getContacts();
      List fetchedBackCards = await getBack();
      setState(() {
          ...
          Container(
            decoration: const BoxDecoration(
              image: DecorationImage(
                image: AssetImage(
                    'assets/images/Design_Thinking_Empathize_Page.png'),
                fit: BoxFit.cover,
              ),
            ),
          ),
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedData is List) {
          print('Successfully decoded data: ${decodedData.length} items');
          return decodedData;
        } else if (decodedData is Map && decodedData.containsKey('error')) {
          throw Exception('Server returned an error: ${decodedData['error']}');
        } else {
          throw Exception('Invalid data format received: $decodedData');
        }
      } else {
        throw Exception(
            ...
            TextButton(
              ...
              onPressed: () {
                Navigator.of(context).pushReplacement(
                  MaterialPageRoute(builder: (context) => const Define()),
                );
              },
            ),
            TextButton(
                  ...
                  ? Column(
                      ...
                      children: [
                        ...
                        widget.frontImage.isNotEmpty
                            ? Image.asset(
                                'assets/images/DT/${widget.frontImage}',
                                width: 100,
                                height: 100,
                                fit: BoxFit.cover,
                              )
